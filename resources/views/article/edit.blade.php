...
@section('content')
    ...
    <form id="article_form" class="form-horizontal" action="{{ route('article.update', $article->id) }}" enctype="multipart/form-data" method="POST" data-parsley-validate>
        {{ csrf_field() }}
        {{ method_field('PUT') }}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input value="{{ old( 'title', $article->title) }}" type="text" class="form-control" id="title" name="title" maxlength="255" required>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input value="{{ old( 'slug', $article->slug) }}" type="text" class="form-control" id="slug" name="slug" maxlength="255" required>
            </div>
        </div>
        ...
        <div class="form-group text-center">
            <div class="col-sm-10 col-sm-offset-2">
                <img id="image_preview" src="{{ asset('storage/'.$article->image) }}">
            </div>
        </div>
                        ...
                        @foreach($category_list as $category)
                            <optgroup label="{{ $category->name }}">
                                @forelse($category->children as $child)
                                    <option value="{{ $child['id'] }}" {{ ($child['id']==$article->category_id) ? 'selected' : '' }}>{{ $child['name'] }}</option>
                                @empty
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <label class="radio-inline">
                    <input type="radio" name="status" id="inlineRadio1" value="1" {{ ( $article->status == 1 ) ? "checked" : "" }}> 開放
                </label>
                <label class="radio-inline">
                    <input type="radio" name="status" id="inlineRadio2" value="0" {{ ( $article->status == 0 ) ? "checked" : "" }}> 關閉
                </label>
            </div>
        </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#tag_id').val([{{ $article_tag }}]).trigger('change');
